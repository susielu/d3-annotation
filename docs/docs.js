...
$(document).ready(function () {
  ...
  d3.selectAll(".icons .presets .types img").on("click", function () {
    ...
    d3.select(
      `.icons img[data-section="note:align"][data-setting="${options.note.align}"]`
    ).classed("active", true);

    if (options.note.lineType) {
      d3.select(
        `.icons img[data-section="note:lineType"][data-setting=${options.note.lineType}]`
      ).classed("active", true);
      d3.selectAll(".icons .orientation").classed("hidden", true);
    } else {
      d3.select(
    ...
    d3.select(
      `.icons img[data-section="connector:type"][data-setting=${options.connector.type}]`
    ).classed("active", true);
